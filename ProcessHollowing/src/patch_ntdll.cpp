...
bool Patch::NtManageHotPatch32(HANDLE hProcess)
{
...
bool Patch::NtManageHotPatch64(HANDLE hProcess)
{
    HMODULE hNtdll = GetModuleHandleA("ntdll");
    if (!hNtdll) return false; // should never happen
...
bool Patch::ZwQueryVirtualMemory(HANDLE hProcess, LPVOID module_ptr)
{
#ifndef _WIN64
    return false;
#else
...
bool Patch::GetWindowsBuildNumber(DWORD& outBuild)
{
...
bool Patch::IsWindows11OrNewerBuild(Win11Release minBuild)
{
    DWORD build = 0;
    if (!GetWindowsBuildNumber(build))
        return false;

    return build >= static_cast<DWORD>(minBuild);
}

bool Patch::ApplyNtManageHotPatch(bool is32BitProcess, HANDLE processHandle)
{
    constexpr DWORD kMinBuild = static_cast<DWORD>(Win11Release::Win11_24H2);
    const char* arch = is32BitProcess ? "x86" : "x64";

    DWORD build = 0;
    if (!GetWindowsBuildNumber(build) || build < kMinBuild)
        return true;

    const bool ok = is32BitProcess
        ? NtManageHotPatch32(processHandle)
        : NtManageHotPatch64(processHandle);

    printf("%s NtManageHotPatch (%s): %s\n",
        ok ? "[+]" : "[-]",
        arch,
        ok ? "patched" : "patch failed");
